...
当前支持两种把试卷加入下载队列的方式：
- 直接粘贴 `PDF 查看器链接`，适合一个文件一个文件添加
- 直接输入 `课程代码`，适合按一门课批量发现并下载全部试卷
...
- 两种入队方式：既支持逐条导入 PDF Viewer 链接，也支持按课程代码批量发现试卷
- 课程代码批量发现：支持一次输入多门课程代码，例如 `EEE205 CPT210 CAN202`
- 登录态检测：根据程序保存的 ETD 站点真实登录态显示红/绿提示，减少误操作
- 会话复用：程序自管 ETD 登录会话，避免每次都重新认证
- 交互优化：支持回车直接加入队列、剪贴板批量导入、重复链接自动跳过
- 文件安全：自动生成文件名并处理重名冲突
...
2. **登录 ETD**：
   ...
   - 当窗口顶部状态提示变成绿色时，表示当前程序会话已经可直接使用，不需要再次登录

3. **把试卷加入下载队列**：
   - 方式一：将 PDF Viewer URL 粘贴到输入框中，直接按 `Enter`，或点击“加入下载队列”
   - 方式二：输入一门或多门课程代码，例如 `EEE205 CPT210 CAN202`，然后点击“发现试卷并加入队列”
   - 课程代码模式会先检索并解析可下载记录，再把结果加入下方任务表格
   - 两种方式都会先把任务加入下载队列，不会立刻开始下载
   - 重复链接会自动跳过

4. **设置保存位置**：
   - 点击“选择目录”选择文件保存的目标文件夹

5. **开始下载队列**：
   - 确认下载队列中已有任务后，点击“开始下载队列”
...
1. 使用任何下载功能前，都建议先点击“先登录 ETD”，让程序保存自己的登录会话
2. 当前支持两种添加任务方式：`PDF 查看器链接` 和 `课程代码`
3. 课程代码模式依赖程序自己的 ETD 网站登录态；如果顶部仍是红色提示，先不要开始发现和下载
4. viewer URL 中包含时效性签名，请在有效期内及时下载
5. 下载请求的 IP 与登录上下文应保持一致
6. 本工具仅供 XJTLU 学生学习复习使用
...
1. 先在程序中点击“先登录 ETD”，并在程序浏览器中完成登录
2. 回到主窗口，在“通过课程代码批量添加试卷”输入框中输入课程代码
3. 支持一次输入多门课，例如 `EEE205 CPT210 CAN202`
4. 点击“发现试卷并加入队列”，程序会自动检索并把可下载记录加入下载队列
5. 确认任务表格内容无误后，再点击“开始下载队列”

如果你想先在网页里确认课程是否存在，可以访问：
- [ETD Past Exam Papers 搜索页](https://etd.xjtlu.edu.cn/index.html#/SearchDetail/EXAMXJTLU)
...
- PySide6 (桌面 GUI)
- Playwright (浏览器自动化)
- ETD 搜索接口解析（课程代码 -> 记录详情 -> viewer URL）
...
- 你是否已经点击“先登录 ETD”并在程序浏览器中登录
- 顶部登录状态是否已经变成绿色
- 你是否已经先把任务加入下载队列，再点击“开始下载队列”
